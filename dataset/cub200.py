...
class CustomDataset(Dataset):
    def __init__(self, feature_dir, label_dir):
        ...
        self.feature_files = [f"{i}.npy" for i in range(5996)]
        self.label_files = [f"{i}.npy" for i in range(5996)]
...
def build_cub200(args, transform):
    return ImageFolder(args.data_path, transform=transform)

def build_cub200_code(args):
    feature_dir = f"{args.code_path}/cub200{args.image_size}_codes"
    label_dir = f"{args.code_path}/cub200{args.image_size}_labels"

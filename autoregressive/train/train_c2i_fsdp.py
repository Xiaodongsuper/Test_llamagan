...
def setup_fsdp_sync(model: nn.Module, args: argparse.Namespace, device) -> FSDP:
    """
    Setup Fully Sharded Data Parallel (FSDP) configuration for distributed training
    
    Args:
        model: Model to be distributed
        args: Training configuration arguments  
        device: GPU device
    
    Returns:
        Model wrapped with FSDP configuration
        
    Features:
    - Configure FSDP auto wrapping policy
    - Set sharding strategy (full/grad/hybrid sharding)
...
def creat_optimizer_by_name(model, weight_decay, learning_rate, betas, global_rank, logger):
    ...
    Features:
    - Separate parameters that need weight decay from those that don't
    - Create parameter groups
    - Configure AdamW optimizer (optionally use fused implementation)
...
def main(args):
    ...
    - Initialize model and load checkpoint (if exists)
    - Create optimizer
    - Prepare data loader
    - Execute training loop:
      - Forward pass and loss computation
      - Backward pass and gradient update
      - Periodically log training status
    ...
    print(f"class_num: {args.num_classes}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cloud_save_path", type=str, required=True, help='please specify a cloud disk path, if not, local path')
    parser.add_argument("--no_local_save", action='store_true', help='no save checkpoints to local path for limited disk volume')
    parser.add_argument("--gpt_model", type=str, choices=list(GPT_models.keys()), default="GPT-B")
    parser.add_argument("--gpt_resume", type=str, default=None, help="model, optimizer and argument path for resume training")
    parser.add_argument("--gpt_type", type=str, choices=['c2i', 't2i'], default="c2i", help="class-conditional or text-conditional")
    parser.add_argument("--vocab_size", type=int, default=16384, help="vocabulary size of visual tokenizer")
    parser.add_argument("--ema", action='store_true', help="whether using ema training")
    parser.add_argument("--cls_token_num", type=int, default=1, help="max token number of condition input")
    parser.add_argument("--dropout_p", type=float, default=0.1, help="dropout_p of resid_dropout_p and ffn_dropout_p")
    parser.add_argument("--token_dropout_p", type=float, default=0.1, help="dropout_p of token_dropout_p")
    parser.add_argument("--drop_path_rate", type=float, default=0.0, help="using stochastic depth decay")
    parser.add_argument("--results_dir", type=str, default="results")
    parser.add_argument("--dataset", type=str, default='cub200_code')
    parser.add_argument("--image_size", type=int, choices=[256, 384, 448, 512], default=256)
    ...
    parser.add_argument("--global_batch_size", type=int, default=96)
    parser.add_argument("--global_seed", type=int, default=0)
    parser.add_argument("--num_workers", type=int, default=24)
    parser.add_argument("--log_every", type=int, default=100)
    parser.add_argument("--ckpt_every", type=int, default=1000)
    parser.add_argument("--gradient_accumulation-steps", type=int, default=1)
    parser.add_argument("--mixed_precision", type=str, choices=["fp32", "tf32", "fp16", "bf16"], default='bf16') 
    parser.add_argument("--data_parallel", type=str, choices=["sdp", "fsdp", "hsdp"], default="fsdp")
    parser.add_argument("--grad_precision", type=str, choices=["fp32", "fp16", "bf16"])
    parser.add_argument("--wandb_project", type=str, default='c2i_fsdp')
